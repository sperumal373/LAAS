...
for i, l in enumerate(lines):
    if "PLANS TAB" in l:
        plans_idx = i
        break
print(f"PLANS TAB at line {plans_idx}")

before = lines[:plans_idx]

# Build the new plans tab content using actual emoji
P = "\n".join([
'      {/* ======================== PLANS TAB ======================== */}',
'      {tab === "plans" && (',
'        <div style={card}>',
'          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>',
'            <div style={{ fontSize: 16, fontWeight: 800, color: p.text }}>{"\U0001F4CB"} Migration Plans</div>',
'            <button onClick={loadPlans} style={btnOutline}>{"\U0001F504"} Refresh</button>',
'          </div>',
'',
...
'                          <button onClick={() => delPlan(plan.id)} style={{ background: "transparent", border: "none", cursor: "pointer", color: p.red, fontWeight: 700, fontSize: 11 }}>{"\U0001F5D1\uFE0F"}</button>',
'                        </td>',
'                      </tr>',
...
'                                    <div style={{ width: 30, height: 30, borderRadius: "50%", background: isReached ? sc.color : `${p.textMute}22`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, border: isCurrent ? `2.5px solid ${sc.color}` : "2.5px solid transparent", boxShadow: isCurrent ? `0 0 12px ${sc.color}44` : "none", transition: "all .3s", animation: isCurrent && ["executing","migrating","validating","preflight_running"].includes(plan.status) ? "pulse 2s infinite" : "none" }}>{sc.icon}</div>',
...
'                                  {plan.vm_list.map((vm, vi) => <div key={vi} style={{ padding: "4px 10px", borderRadius: 6, background: `${p.border}22`, fontSize: 11, fontWeight: 600 }}>{vm.name || vm}</div>)}',
'                                </div>',
'                              </div>',
'                            )}',
...
'                                {actions.map((act, ai) => (',
'                                  <button key={ai} onClick={() => act.confirm ? (confirm("Are you sure?") && act.fn()) : act.fn()} style={{ padding: "7px 16px", borderRadius: 7, border: `1px solid ${act.color}`, background: `${act.color}12`, color: act.color, fontWeight: 700, fontSize: 11.5, cursor: "pointer", transition: "all .15s" }} onMouseEnter={e => { e.target.style.background = act.color; e.target.style.color = "#fff"; }} onMouseLeave={e => { e.target.style.background = `${act.color}12`; e.target.style.color = act.color; }}>{act.label}</button>',
'                                ))}',
'                              </div>',
'                            )}',
'                            {(() => {',
...
'                                  <div style={{ padding: "8px 12px", background: `${p.border}33`, fontSize: 11, fontWeight: 700, color: p.text }}>{"\U0001F4DC"} Activity Log ({events.length})</div>',
...
result = "".join(before) + P
open(M, 'w', encoding='utf-8').write(result)
print(f"Done. New file: {result.count(chr(10))} lines")