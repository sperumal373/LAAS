...
# C1
c1old = b'const [liveStatus, setLiveStatus] = useState(' + q + q + b');'
raw = raw.replace(c1old, c1old + b'\r\n  const [liveMtvStatus, setLiveMtvStatus] = useState(null);', 1)
print('1 OK')

# C2
c2old = b'setLiveStatus(r.status || ' + q + q + b');\r\n        if ([' + q + b'completed' + q + b', ' + q + b'failed' + q + b', ' + q + b'cancelled' + q + b', ' + q + b'rolled_back' + q + b']'
c2new = b'setLiveStatus(r.status || ' + q + q + b');\r\n        if (r.mtv_status) setLiveMtvStatus(r.mtv_status);\r\n        if ([' + q + b'completed' + q + b', ' + q + b'failed' + q + b', ' + q + b'cancelled' + q + b', ' + q + b'rolled_back' + q + b']'
raw = raw.replace(c2old, c2new, 1)
print('2 OK')

# C3
c3old = b'setLiveStatus(r.status || ' + q + q + b');\r\n    }).catch(() => {});'
c3new = b'setLiveStatus(r.status || ' + q + q + b');\r\n      if (r.mtv_status) setLiveMtvStatus(r.mtv_status);\r\n    }).catch(() => {});'
raw = raw.replace(c3old, c3new, 1)
print('3 OK')

# C4: MTV pipeline stages
marker = b'</div>\r\n                            )}\r\n                            {actions.length > 0 && ('
assert raw.count(marker) == 1, f'Expected 1 marker, found {raw.count(marker)}'
