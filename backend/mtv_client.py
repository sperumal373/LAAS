...
def get_plan_status(cluster, plan_name):
    ...
    for vm in migration.get("vms", []):
        ...
        for step in vm.get("pipeline", []):
            ...
            if step.get("name") in ("DiskTransfer", "DiskTransferV2v", "DiskAllocation"):
                total_bytes += t
                completed_bytes += c
...
def orchestrate_migration(plan_db_row, log_fn=None):
    ...
    options = json.loads(plan_db_row.get("options") or "{}") if isinstance(plan_db_row.get("options",""), str) else (plan_db_row.get("options") or {})
    ...
    # 4. Auto-discover actual VM networks and datastores
    ...
    for v in vm_ids:
        if v.get("cbt_disabled"):
            use_warm = False
            _log(f"VM '{v['name']}' has CBT disabled - forcing COLD migration")
            break
    _log(f"Migration mode: {'WARM' if use_warm else 'COLD'}")
    ...
    tgt_ns = options.get("target_namespace", "openshift-mtv") or "openshift-mtv"
    skip_convert = options.get("skip_guest_conversion", False)
    preserve_ips = options.get("preserve_static_ips", False)
    create_mtv_plan(cluster, plan_name, source_provider, nw_name, st_name, vm_ids, 
                    target_namespace=tgt_ns, warm=use_warm)
