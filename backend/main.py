...
def list_available_playbooks(group_id: int, aap_id: int = None, search: str = None, u=Depends(require_role("admin","operator"))):
    """List job templates from AAP instances. If aap_id given, fetch from that one only."""
    from ansible_client import list_aap_instances, get_aap_instance, get_aap_job_templates
    playbooks = []
    if aap_id:
        instances = [get_aap_instance(aap_id)]
    else:
        # Get all instances WITH password using get_aap_instance
        safe_list = list_aap_instances()
        instances = [get_aap_instance(i["id"]) for i in safe_list]
    for inst in instances:
        if not inst:
            continue
        try:
            templates = get_aap_job_templates(inst, search=search)
            for t in templates:
