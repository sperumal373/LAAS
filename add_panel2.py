...
# Find the PLANS TAB line dynamically
import re
m = re.search(r'(\s*\{/\*.*PLANS TAB.*\*/\})', text)
if not m:
    print('No PLANS TAB marker found')
    exit(1)
marker = m.group(1)
print('Found marker:', repr(marker[:60]))

if 'POST-MIGRATION TASKS PANEL' in text:
