...
if (lines[freeclrIdx].includes('thresh_used_warn')) {
  console.log('freeClr already patched — skipping');
} else {
...
if (alreadyHasModal) {
  console.log('ThresholdModal already exists — skipping');
} else {
for (let i = 0; i < lines.length; i++) {
...
const threshModal = [
  ...
  "      return alert('Snapshot: Warning days must be less than Critical (1-3650)');",
  ...
  "        <span style={{fontSize:16,fontFamily:'monospace',color:p.textMute}}>{icon}</span>",
  ...
  "        <span style={{color:p.textMute,fontSize:16,padding:'0 2px'}}>&#8594;</span>",
  ...
  "          <div style={{fontSize:10,color:p.textMute,marginTop:3}}>{unit}</div></div>",
  "      </div>",
  "      <div style={{flex:'0 0 88px',textAlign:'right',fontSize:11,color:p.textMute}}>",
  ...
  "            Set <b>Warning</b> (yellow) and <b>Critical</b> (red) thresholds. Saved to browser storage &#8212; applies immediately.",
  "          </div>",
  "          <Cat icon='CPU' title='CPU Usage'       sub='% utilised'        warnVal={cpuWarn}  warnSet={setCpuWarn}  critVal={cpuCrit}  critSet={setCpuCrit}  unit='%'/>",
  "          <Cat icon='RAM' title='RAM Usage'       sub='% utilised'        warnVal={ramWarn}  warnSet={setRamWarn}  critVal={ramCrit}  critSet={setRamCrit}  unit='%'/>",
  "          <Cat icon='DS'  title='Datastore Usage' sub='% utilised'        warnVal={dsWarn}   warnSet={setDsWarn}   critVal={dsCrit}   critSet={setDsCrit}   unit='%'/>",
  "          <Cat icon='SNAP' title='Snapshot Age'   sub='days since created' warnVal={snapWarn} warnSet={setSnapWarn} critVal={snapCrit} critSet={setSnapCrit} unit='days'/>",
  "        </div>",
...
for (let i = vmPageStart; i < vmPageStart + 900; i++) {
  ...
  if (l.includes('summary of current vmware')) {
    summaryIdx = i; break;
  }
}
...
const threshCard = [
  ...
  "                    <div style={{fontSize:11,color:p.textMute,marginTop:1}}>CPU &middot; RAM &middot; Datastore &middot; Snapshot age alert thresholds</div>",
  "                  </div>",
...
changes++;
console.log('Threshold card inserted. Total changes:', changes);
